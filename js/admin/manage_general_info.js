...
$(document).ready(function(){
	...
	$(document).on('click' , '.global-list-status-icon' , function(e){
		...
		var id = $(this).find('.fa').data('general_info_id');
		var status = $(this).find('.fa').data('status_type');
		confirmAction(message , function(c){
			if(c){
				$.ajax({
					url : 'manage_general_info/update_status',
					type : 'POST',
					data : {'general_info_id' : id , 'status' : status , csrf_test_name: $.cookie('csrf_cookie_name')},
					success : function(){
	...
	if(pageType == 'add_edit')
	{
		...
		$('#activityDetails').validate({
			...
			messages : {
				name : {
					required : please_enter_dynamic.replace('**field**' , 'General info name')
				},
				centre_id : {
	...
	$(document).on('click' , '.moveUp' , function(){
		...
		$.ajax({
			url : 'manage_general_info/change_sequence',
			type : 'POST',
			data : {
						'currentId' : currentId ,
			...
			success : function(response){
				window.location = 'manage_general_info';
			}
		});
	});
