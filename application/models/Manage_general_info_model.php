<?php
	...
	*@author	S.D
	*/
	class Manage_general_info_model extends CI_Model
	{
		...
		*/
		...
		{
			$resultData = array();
			$colomnArr = array('a.plus_general_info_id' , 'a.name' , 'b.nome_centri' , 'a.added_date' , 'a.status' , 'a.front_image' , 'a.sequence');
			$this->db->select(implode(',' , $colomnArr));
			$this->db->from(TABLE_GENERAL_INFO . ' a');
			...
			if(!empty($result))
			{
				...
				foreach($result as $key => $value)
				{
					...
					$actionStr .= '<a class="btn btn-xs btn-info btn-wd-24" href="'.base_url().'manage_general_info/add_edit/'.$value['plus_general_info_id'].'" data-toggle="tooltip" data-original-title="Edit General info"><span><i class="fa fa-pencil-square-o" aria-hidden="true"></i></span></a>';
					$actionStr .= '<a class="btn btn-xs btn-danger btn-wd-24" href="'.base_url().'manage_general_info/delete/'.$value['plus_general_info_id'].'" onclick="return confirm_delete()" data-toggle="tooltip" data-original-title="Delete general info"><span><i class="fa fa-trash-o" aria-hidden="true"></i></span></a>';
					$statusClass = ($value['status'] == 1) ? 'fa-check-square-o' : 'fa-square-o';
					$actionStr .= '<a data-toggle="tooltip" data-original-title="Change Status for activity" class="btn btn-xs btn-danger btn-wd-24 global-list-status-icon"><span><i class="fa '.$statusClass.'" aria-hidden="true" data-toggle="modal" data-target="#juniorCentreStatus" data-status_type = '.$value['status'].' data-general_info_id = '.$value['plus_general_info_id'].' ></i></span></a>';
					$actionStr .="</div>";
