	...
	*/
	class Activity_model extends CI_Model
	{
		...
		*/
		...
		/**
		*This function is used to get the general information details from database
		*
		*@access public
		*@author S.D
		*@since 7th June, 2016
		*@param Interger $centreId : The centre id
		*@return NONE
		*/
		public function getgeneralInfoDetails($centreId = NULL)
		{
			$returnArr = array();
			$result = $this->db->select("a.plus_general_info_id as id , a.name , a.description , date_format(a.added_date , '%d-%m-%Y') as date ,
										 a.show_type , a.show_text , b.image_name")
							->from(TABLE_GENERAL_INFO.' a')
							->join(TABLE_ACTIVITY_PHOTO_GALLERY.' b' , 'a.front_image = b.activity_photo_gallery_id' , 'left')
							->where('a.centre_id' , $centreId)
							->where('a.status' , 1)
							->where('a.delete_flag' , 0)
			...
			if(!empty($result))
			{
				foreach($result as $key => $value)
				{
					...
					$returnArr[$key]['fixed_background_image'] =  base_url().'images/bg_image.jpg';
					$returnArr[$key]['image_name'] = ADMIN_PANEL_URL.ACTIVITY_PHOTOGALLERY_IMAGE_PATH.$value['image_name'];
					$returnArr[$key]['files'] = $this->getGenaralInfoFiles($value['id']);
				}
			}
			return $returnArr;
		}
		...
		/**
		*This function is used to get the general information files
		*
		*@access private
		*@author S.D
		*@since 7th June, 2016
		*@param Interger $id : general information id
		*@return NONE
		*/
		private function getGenaralInfoFiles($id = NULL)
		{
			$returnArr = array();
			$result = $this->db->select('file_name')
							->where('plus_general_info_id' , $id)
							->get(TABLE_GENERAL_INFO_FILE)->result_array();
			if(!empty($result))
			{
				foreach($result as $key => $value)
				{
					$returnArr[$key]['filePath'] = ADMIN_PANEL_URL.GENERAL_INFO_FILE_PATH.$value['file_name'];
					$returnArr[$key]['fileExtension'] = pathinfo($value['file_name'] , PATHINFO_EXTENSION);
				}
			}
			return $returnArr;
