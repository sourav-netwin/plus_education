<?php
	class Admin_model extends CI_Model
	{
		...
		function getCourseDetails($start = NULL , $length = NULL , $search_string = NULL , $order_column = NULL , $order_dir = NULL , $languageId = 1)
		{
			...
			if(!empty($result))
			{
				...
				foreach($result as $value)
				{
					...
					$resultData[] = array(
						0 => $siNo++,
						1 => "<img src = '".base_url()."uploads/course/".$value['course_image']."' width = 180 height = 50 />",
						2 => $value['course_name'],
		...
		function addCourse($data = NULL , $fileName = NULL)
		{
			$insertData = array(
		...
		{
			$this->db->select('a.course_master_id , a.course_image , b.course_name , b.corse_description , b.language_id');
			$this->db->from(TABLE_COURSE_MASTER . ' a');
			$this->db->join(TABLE_COURSE_LANGUAGE . ' b' , 'a.course_master_id = b.course_id AND b.language_id = '.$languageId , 'left');
		...
		//This function is used to update program data in DB
		function updateCourseData($id = NULL , $data = NULL , $fileName = NULL)
		{
			$updateData = array(
				'course_image' => $fileName
			);
